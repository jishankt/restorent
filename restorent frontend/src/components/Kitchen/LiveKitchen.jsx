...
function LiveKitchen() {
    ...
    const liveKitchenOrders = savedOrders.filter(order =>
        order.cartItems.some(item => item.kitchenType === "Live Kitchen")
    );

    const groupedOrders = liveKitchenOrders.reduce((groups, order) => {
    ...
    const handleInformBearer = () => {
        const preparedOrders = liveKitchenOrders.map(order => ({
            ...order,
                        ...
                        <tbody>
                            {Object.entries(groupedOrders).map(([customer, orders]) =>
                                orders.map((order, orderIndex) =>
                                    order.cartItems
                                        .filter(item => item.kitchenType === "Live Kitchen")
                                        .map((item, itemIndex) => (
