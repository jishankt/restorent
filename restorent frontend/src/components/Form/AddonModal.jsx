...
/**
 * Standardized Addon Modal for Item Creation
...
const AddonModal = ({
  ...
  const [addonType, setAddonType] = useState('existing'); // 'existing' or 'new'
  ...
  useEffect(() => {
    if (addonData) {
      setData({
        ...data,
        ...addonData,
        branch_names: addonData.branch_names || [],
        branch_prices: addonData.branch_prices || {},
        variants: addonData.variants || data.variants
      });
      setAddonType(addonData.selectedId ? 'existing' : 'new');
      if (addonData.selectedId) {
        const item = allItems.find(i => i._id === addonData.selectedId);
      ...
      setAddonType('existing');
      setSearchTerm('');
    }
  }, [addonData, isOpen, isCompanyAdmin, userBranch, allItems]);
  ...
  const handleImageUpload = (e, variant = null, subField = null) => {
    ...
    setCropTarget({ field: 'addon_image', subField, variantId: variant, isModal: true });
  ...
  const handleSubmit = (e) => {
    ...
    if (!finalData.name1?.trim()) return alert("Addon name is required");
    onSave(finalData);
  };
    ...
    <div className="modal-overlay" style={{
      ...
      <div className="modal-content" style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
          <h3 style={{ margin: 0, color: '#2c3e50' }}>{addonData ? 'Edit Addon' : 'Add New Addon'}</h3>
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            {data.imagePreview || data.addon_image ? (
              <img 
                src={data.imagePreview || (data.addon_image?.startsWith('data:') ? data.addon_image : `${baseUrl}/api/images/${data.addon_image}`)} 
                alt="Addon Preview" 
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            <label className="btn-primary" style={{ 
              ...
              <FaImage /> {data.addon_image ? 'Change' : 'Upload Image'}
              <input type="file" accept="image/*" onChange={handleImageUpload} style={{ display: 'none' }} />
            </label>
            {(data.imagePreview || data.addon_image) && (
              <>
                <button type="button" onClick={() => handleEditImage(data.imagePreview || data.addon_image, 'addon_image', null, null, null, true)} style={{ padding: '6px 15px', fontSize: '0.85rem', background: '#2ecc71', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Edit</button>
                <button type="button" onClick={() => setData({ ...data, addon_image: '', imagePreview: '' })} style={{ padding: '6px 15px', fontSize: '0.85rem', background: '#e74c3c', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Delete</button>
              </>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ marginBottom: '15px' }}>
            <label style={{ display: 'block', marginBottom: '5px', fontWeight: '600' }}>Addon Type</label>
            <select 
              value={addonType} 
            ...
            >
              ...
              <option value="new">Create New Addon</option>
            </select>
          </div>
          ...
          {addonType === 'existing' ? (
            ...
            <div style={{ marginBottom: '15px' }}>
              <label style={{ display: 'block', marginBottom: '5px', fontWeight: '600' }}>New Addon Name *</label>
              <input 
                type="text" 
                name="newName" 
                value={data.newName} 
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '20px' }}>
            ...
            {!getFieldHidden?.("ingredients", false, "addons") && (
              <button type="button" onClick={() => navigate("/add-ingredients-nutrition", { state: { formData: data, isEditing: !!addonData, type: "addon" } })} style={{
